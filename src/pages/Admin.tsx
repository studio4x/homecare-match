...
const Admin = () => {
  ...
  const checkAdminStatus = async () => {
    try {
      setLoading(true);
      const { count } = await supabase.from("profiles").select("*", { count: 'exact', head: true }).eq("is_admin", true);
  ...
  const handleReject = async () => {
    ...
    if (error) {
      ...
      toast.success("Perfil reprovado. O usuário foi notificado para reenviar os documentos.");
    ...
    try {
      await supabase.functions.invoke('verification-result', {
        body: {
          ...
          // userEmail: profileToNotify.email // O e-mail não está na tabela profiles, precisaria de um join ou buscar em auth.users
        }
      });
    } catch (e) {
  ...
  if (!session) return <Layout><div className="flex min-h-[calc(100vh-16rem)] items-center justify-center py-12 px-4 bg-secondary/20"><div className="w-full max-w-md space-y-8 rounded-2xl border border-border bg-card p-8 shadow-card"><div className="text-center"><Lock className="mx-auto h-12 w-12 text-primary" /><h2 className="mt-6 text-3xl font-bold">Painel de Gestão</h2><p className="mt-2 text-sm text-muted-foreground">Acesse com sua conta de administrador.</p></div><AuthForm mode={adminExists ? "login" : "register"} allowRegister={!adminExists} /></div></div></Layout>;
  if (!isAdmin) return <Layout><div className="flex min-h-[calc(100vh-16rem)] items-center justify-center py-12 px-4 bg-secondary/20"><div className="w-full max-w-md text-center space-y-6 rounded-2xl border border-destructive/20 bg-card p-8 shadow-card"><AlertTriangle className="mx-auto h-16 w-16 text-destructive" /><h2 className="text-2xl font-bold">Acesso Restrito</h2><p className="text-muted-foreground">O usuário <strong>{user?.email}</strong> não possui acesso administrativo.</p><div className="flex flex-col gap-3"><Button onClick={checkAdminStatus} variant="outline" className="gap-2"><RefreshCw className="h-4 w-4" /> Tentar Novamente</Button><Button variant="ghost" onClick={signOut} className="text-destructive">Sair e usar outra conta</Button></div></div></div></Layout>;
      ...
      <div className="min-h-screen bg-secondary/20 py-8">
        <div className="container mx-auto px-4">
          <div className="mb-8 flex items-center justify-between">
            ...
            <div className="flex gap-2"><Button variant="outline" onClick={fetchData} className="gap-2"><RefreshCw className="h-4 w-4" /> Atualizar</Button><Button variant="ghost" onClick={signOut} className="gap-2 hover:text-destructive"><LogOut className="h-4 w-4" /> Sair</Button></div>
          </div>
          ...
          <Tabs defaultValue="verifications" className="space-y-6">
                      ...
                      <TableRow key={p.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button variant="outline" size="sm" asChild className="h-7 text-xs"><a href={p.id_document_url} target="_blank" rel="noopener noreferrer">Ver RG/CNH</a></Button>
                            <Button variant="outline" size="sm" asChild className="h-7 text-xs"><a href={p.prof_registration_url} target="_blank" rel="noopener noreferrer">Ver Registro</a></Button>
                          </div>
                        </TableCell>
            ...
            {/* Outras abas (Usuários, Planos) permanecem aqui */}
          </Tabs>
        </div>
      </div>
      ...
      <Dialog open={rejectionModalOpen} onOpenChange={setRejectionModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Descreva o motivo da reprovação. O profissional será notificado para corrigir e reenviar os documentos.</DialogDescription>
          </DialogHeader>
          <div className="py-4">
