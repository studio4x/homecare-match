...
import { evaluateOnboardingCondition } from "../_shared/onboarding-conditions.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    for (const instance of flows) {
      ...
      if (shouldSend) {
        // send email
        const { data: userData } = await supabase.auth.admin.getUserById(instance.user_id);
        const userEmail = userData?.user?.email;

        if (userEmail) {
          const { success, messageId, error } = await sendEmail({
            to: userEmail,
            subject: step.email_templates.subject || "HomeCare Match",
            html: step.email_templates.html_content || "",
            text: step.email_templates.text_content || "",
          });
