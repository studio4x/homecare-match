    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (userId && planId) {
        ...
        await supabaseAdmin.from('notifications').insert({
          ...
          content: `Sua assinatura do plano \${planId} foi ativada com sucesso. Aproveite todos os recursos!`,
          link: "/dashboard",
          type: 'success'
        });
        ...
        await supabaseAdmin.from('notifications').insert({
          ...
          content: `Seu acesso ao curso "\${course?.title || courseSlug}" foi liberado. Bons estudos!`,
