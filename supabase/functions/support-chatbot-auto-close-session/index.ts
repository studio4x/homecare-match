...
serve(async (req) => {
  ...
  try {
    ...
    const { data: session, error: sessionError } = await supabaseAdmin
      ...
      .select("id,user_id,visitor_hash,user_closed_session,auto_closed_session")
      .eq("id", sessionId)
      .maybeSingle();
    ...
    const isOwner =
      ...
      (!userId && !session.user_id && visitorHashCandidates.includes(String(session.visitor_hash || "")));
    if (!isOwner) {
      console.warn("[support-chatbot-auto-close-session] auto close sem owner-match", {
    ...
    if (session.user_closed_session || session.auto_closed_session) {
      return new Response(JSON.stringify({ ok: true, already_closed: true, session_id: sessionId }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    console.error("[support-chatbot-auto-close-session] erro:", error?.message || error);
    return new Response(
      JSON.stringify({
        error: error?.message || "Erro ao encerrar sessão por inatividade.",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
