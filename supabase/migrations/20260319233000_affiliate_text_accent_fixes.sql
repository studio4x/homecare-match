-- Affiliate Program v1 - ajustes de acentuação em descrições e templates
...
BEGIN
  ...
  WHILE v_block <= v_target_blocks LOOP
    ...
    VALUES (
      ...
      v_amount,
      'BRL',
      format('Bônus por %s cadastros completos validados', v_block * 10),
      'signup_milestone',
      v_event_id,
      v_entry_status,
      jsonb_build_object(
...
SET
  sample_message = REPLACE(sample_message, 'Publico: ', 'Público: '),
  var2_default = CASE
    WHEN var2_default = 'Publico nao informado' THEN 'Público não informado'
    ELSE var2_default
  END
WHERE event_type = 'affiliate_interest_admin';

UPDATE public.affiliate_commission_ledger
SET description = 'Comissão por cadastro completo (docs validados)'
WHERE description = 'Comissao por cadastro completo (docs validados)';

UPDATE public.affiliate_commission_ledger
SET description = 'Comissão recorrente por pagamento válido do indicado'
WHERE description = 'Comissao recorrente por pagamento valido do indicado';

UPDATE public.affiliate_commission_ledger
SET description = REGEXP_REPLACE(description, '^Bonus por ', 'Bônus por ')
WHERE description ~ '^Bonus por ';
